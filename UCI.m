...
for i = 1:length(allfolders)
    ...
    for fold = 1:k
        ...
        % Train
        tic; tic;
        [W,U] = CPLS(X,Y,M,R,lambda,lengthscale,sweeps);
        trainTime(fold) = toc;
        trainError(fold) = sqrt(mean((CPPredict(X, W, U, lengthscale)-Y).^2))/std(Y); 
        
        % Implicit approximation error
        approxError(fold) = approximationError(X,U,lengthscale);
        ...
        Y = (Y-YMean)./YStd;
        X = X(:,nonNanCols);
        testError(fold) = sqrt(mean((CPPredict(X, W, U, lengthscale)-Y).^2))/std(Y); 
